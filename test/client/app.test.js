'use strict';

const test = require('node:test');
const assert = require('node:assert/strict');

const { createDocument } = require('./helpers');

test('stub page shows the app title', () => {
  const { doc } = createDocument();
  const title = doc.createElement('h1');
  title.textContent = 'Gachabooru';
  doc.body.append(title);

  const found = doc.querySelector('h1');
  assert.ok(found);
  assert.equal(found.textContent, 'Gachabooru');
});

test('server status reflects a healthy API', async () => {
  const { doc } = createDocument();
  const status = doc.createElement('div');
  status.id = 'server-status';
  doc.body.append(status);

  const fakeFetch = async () => ({
    ok: true,
    json: async () => ({ ok: true }),
  });

  const app = require('../../public/app').createApp({
    document: doc,
    fetch: fakeFetch,
  });
  await app.init();

  assert.equal(status.textContent, 'online');
});

test('banking a roll refreshes the gallery', async () => {
  const { doc } = createDocument();
  for (const id of ['banner-section', 'roll-section', 'gallery-section']) {
    const section = doc.createElement('section');
    section.id = id;
    doc.body.append(section);
  }

  const fetchStub = async (url) => {
    if (url === '/api/health') {
      return { ok: true, json: async () => ({ ok: true }) };
    }
    return { ok: true, status: 200, json: async () => ({}) };
  };

  let onBankedCallback = null;
  let galleryLoads = 0;

  const app = require('../../public/app').createApp({
    document: doc,
    fetch: fetchStub,
    createBannerPicker: null,
    createRoll: ({ onBanked }) => {
      onBankedCallback = onBanked;
      return {
        el: doc.createElement('div'),
        setBanner: () => {},
        loadBalance: () => {},
      };
    },
    createGallery: () => ({
      el: doc.createElement('div'),
      load: () => {
        galleryLoads += 1;
      },
    }),
  });

  await app.init();

  assert.equal(galleryLoads, 1);

  onBankedCallback();
  assert.equal(galleryLoads, 2);
});
